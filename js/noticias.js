...
function getImpact(item, index){
  const base = 62 + ((index * 7) % 27);
...
function renderFeatured(item) {
  ...
  container.innerHTML = `
    <article class="featured-news-card news-center-card">
      <div class="news-center-screen">
        <span class="news-console-label">ERICK ISLAS TECH NEWS CENTER</span>
        <h2>${item.titulo}</h2>
        <p>${item.resumen}</p>
        <div class="news-meta">
          <span>${formatDate(item.fecha)}</span>
          <span>${item.categoria}</span>
          <span>${item.tiempo} lectura</span>
        </div>
      </div>
      <div class="news-feature-action">
        <span class="news-impact">Impacto estimado <b>${getImpact(item, 1)}%</b></span>
...
function renderNews() {
  ...
  grid.innerHTML = filtered.map((item, index) => `
    <article class="news-card news-card-premium" data-category="${item.categoria}">
      <div class="news-thumb-placeholder" aria-hidden="true">
        <span>NEWS</span>
        <small>Miniatura pendiente</small>
      </div>
